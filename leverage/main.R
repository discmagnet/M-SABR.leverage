...
data$new_inning <- data$half_inning != lag(data$half_inning)
data$end_inning <- data$half_inning != lead(data$half_inning)

data <- data[order(data$BAT_HOME_ID),]

data$new_pitcher <- data$PIT_ID != lag(data$PIT_ID)
data$fresh_inning <- data$new_pitcher & data$new_inning

# Manually fix data to replace missing values
data[1,c("new_inning","new_pitcher","fresh_inning")] <- TRUE
data[nrow(data),"end_inning"] <- TRUE

games <- summarise(group_by(data,RESP_PIT_ID),appearances = sum(new_pitcher))
